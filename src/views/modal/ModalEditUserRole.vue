<template>
  <div
    ...
    v-if="isOpen">
    <base-card title="Edit user Role">
      <div class="mt-8">
        <form @submit.prevent="updateUserRole" class="grid grid-cols-1 gap-2 px-8">
          <Label for="name" value="Name" class="text-left" />
          <Input type="text" 
            hidden
            v-model="editedUserRole.id" />          
          <Input type="text" 
            disabled
            v-model="editedUserRole.pegawai.nama" />          
          <Label for="name" value="route to" class="text-left" />
          <input-select
              :options="selectOptions"
              v-model="editedUserRole.roles[0].pivot.role_id"
            />
...
export default {
  props: {
    isOpen: Boolean,
    editedUserRole: Object,
    selectOptions: Array,
    confirm: Boolean, // Tambahkan properti confirm dengan tipe Boolean
  },
  setup(props, { emit }) {
    const updateUserRole = () => {
      // Panggil action updateUserRole dari Vuex dengan data yang akan diubah
      emit('update-user-role', props.editedUserRole);
    };
